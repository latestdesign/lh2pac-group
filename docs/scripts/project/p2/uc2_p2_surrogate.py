...
class MyUncertainSpace(ParameterSpace):
    def __init__(self):
        ...
        self.add_random_variable(
        "bed", "OTUniformDistribution", minimum=400, maximum=700.0
        )
...
# suite
# Define outputs of interest (objective and constraints)
outputs = ["mtom", "tofl", "vapp", "vz", "span", "length", "fm"]

# ### 1. Generate training and test datasets
training_dataset = sample_disciplines(
    disciplines, uncertain_space, outputs, algo_name="OT_OPT_LHS", n_samples=500
)
test_dataset = sample_disciplines(
    disciplines, uncertain_space, outputs, algo_name="OT_FULLFACT", n_samples=10000
)
